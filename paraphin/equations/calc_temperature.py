    ...
    def calc_temperature_loop():
        for i, j in ti.ndrange((1, Nx - 1), (1, Ny - 1)):
            multiplier = dT / (m[i, j] * S[i, j] * ro_w * C_w[i, j] + m[i, j] * (1.0 - S[i, j]) * ro_o * C_o[i, j] +
                               (m * (1.0 - S[i, j]) * Wps + Wp) * ro_p * C_p + (1.0 - m - Wp) * ro_f * C_f) / volume
            ...
            t1 *= area * (m[i, j] * (S[i, j] * K_w + (1.0 - S[i, j]) * K_o) +
                          (m[i, j] * (1.0 - S[i, j]) * Wps[i, j] + Wp) * K_p + (1.0 - m[i, j] - Wp) * K_f)

            t2 *= area * T[i, j] * ro_w * C_w

            t3 *= area * T[i, j] * (ro_o * C_o * (1.0 - Wps) + ro_p * Wps * C_p)
