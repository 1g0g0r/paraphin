from numpy import array, float64, log, sqrt, pi
from taichi import f64, f32

default_type = f64
output_file_name = 'data.pkl'


# Параметры сетки
Nx, Ny = 258, 258  # Число узлов сетки по x и y
X_min, X_max = 0., 1.
Y_min, Y_max = 0., 1.
hx = (X_max - X_min) / Nx
hy = (Y_max - Y_min) / Ny
area = hx * hy
volume = area * 1.0

# Параметры решения
Time_end = 1.
dt = 0.0001
sol_time_step = 0.1

# Физические параметры задачи
S_min = 0.18
S_max = 0.7

Tm = 55  # температура кристализации парафина, [C]
ro_w = 1000  # плотность воды, [кг/м^3]
ro_o = 860  # плотность нефти, [кг/м^3]
ro_p = 900  # плотность парафина, [кг/м^3]
ro_f = 2500  # плотность пласта, [кг/м^3]
qw = 20  / 86400 * log(0.1 / (0.14 * sqrt(hx*hx + hy*hy))) / pi / 2.0 # дебит воды, [м^3/c]
qo = -20 / 86400 * log(0.1 / (0.14 * sqrt(hx*hx + hy*hy))) / pi / 2.0 # дебит нефти, [м^3/c]

# TODO проверить!!
K_p = 0.2  # Теплопроводность парафина, Вт/(м·К)
K_w = 0.6  # Теплопроводность воды, Вт/(м·К)
K_o = 0.12  # Теплопроводность нефти, Вт/(м·К)
K_f = 2  # Теплопроводность пласта, Вт/(м·К)

# данные моделирования кольматации\суффозии
# TODO проверить!!
Diff = 2e-16  # Коэффициент диффузионного осаждения частиц, [м^2/c]
D = 4e-6  # Размер частицы, [м]
betta = 0.01  # Доля блокируемых каналов, [-]
gamma = 0.4  # Отношение радиуса горла к радиусу канала, [-]
Lk =  3e-5 # Длина капилляра, [м]
eta = 1.0  # Коэффициент извилистости, [-]
Cf = 3.e-2  # Коэф-т сопротивления частицы в нефти

# данные инициализации
init_p = 1e6  # [Па]
init_S = 0.0
init_Wo = 1.0
init_Wp = 0.0
init_Wps = 0.0
init_k = 3e-14  # [м^2]
init_m = 0.6
init_T = 40  # [C]
init_qp = 0.0
init_h_sloy = 0.0
r = array([0, 0.000001, 0.000002, 0.000003, 0.000004, 0.000005, 0.000006, 0.000007, 0.000008,
           0.000009, 0.00001, 0.000011, 0.000012, 0.000013, 0.000014, 0.000015, 0.000016, 0.000017,
           0.000018, 0.000019, 0.00002, 0.000021, 0.000022, 0.000023, 0.000024, 0.000025], dtype=float64)

fi_0 = array([0, 0.013, 0.023, 0.031, 0.035, 0.034, 0.027, 0.021, 0.016, 0.018, 0.025, 0.032, 0.041,
              0.052, 0.061, 0.073, 0.082, 0.086, 0.081, 0.07, 0.059, 0.048, 0.035, 0.024, 0.013, 0], dtype=float64)
Nr: int = len(r)
