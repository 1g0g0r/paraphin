...
if use_sparse_matrix:
    ...
    N = res * res
    K = ti.linalg.SparseMatrixBuilder(N, N, max_num_triplets=N * 5)
...
def main():
    ...
    gui = ti.GUI("Stable Fluid", (600, 600))
    md_gen = MouseDataGen()
    ...
    while gui.running:
        ...
        if visualize_c:
            vorticity(velocities_pair.cur)
            data = velocity_curls.to_numpy() * 0.03 + 0.5
            resized_image = ti.tools.imresize(data, 600, 600)
            ...
            data = velocities_pair.cur.to_numpy() * 0.01 + 0.5
            resized_image = ti.tools.imresize(data, 600, 600)
            gui.set_image(resized_image)
